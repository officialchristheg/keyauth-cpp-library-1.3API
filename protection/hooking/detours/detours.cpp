...
static PVOID detour_alloc_region_from_lo(HANDLE hTargetProcess, PBYTE pbLo, PBYTE pbHi)
{
    ...
    for (; pbTry < pbHi;) {
        ...
        if (mbi.State == MEM_FREE && mbi.RegionSize >= DETOUR_REGION_SIZE) {
            ...
            PVOID pv = VirtualAllocEx(hTargetProcess,
                pbTry,
                DETOUR_REGION_SIZE,
...
static PVOID detour_alloc_region_from_hi(handle_t hTargetProcess, PBYTE pbLo, PBYTE pbHi)
{
    ...
    for (; pbTry > pbLo;) {
        ...
        if (mbi.State == MEM_FREE && mbi.RegionSize >= DETOUR_REGION_SIZE) {
            ...
            PVOID pv = VirtualAllocEx(GetCurrentProcess(),
                pbTry,
                DETOUR_REGION_SIZE,
                MEM_COMMIT | MEM_RESERVE,
                PAGE_EXECUTE_READWRITE);
    ...
    PDETOUR_TRAMPOLINE pHi)
{
    ...
    if (pbTry == NULL && pbTarget > (PBYTE)0x40000000) {
        pbTry = detour_alloc_region_from_hi(hTargetProcess, (PBYTE)pLo, pbTarget - 0x40000000);
    }
    // Try looking 1GB above or higher.
    if (pbTry == NULL && pbTarget < (PBYTE)0xffffffff40000000) {
        pbTry = detour_alloc_region_from_lo(hTargetProcess, pbTarget + 0x40000000, (PBYTE)pHi);
    }
    // Try looking 1GB below or higher.
    if (pbTry == NULL && pbTarget > (PBYTE)0x40000000) {
        pbTry = detour_alloc_region_from_lo(hTargetProcess, pbTarget - 0x40000000, pbTarget);
    }
    // Try looking 1GB above or lower.
    if (pbTry == NULL && pbTarget < (PBYTE)0xffffffff40000000) {
        pbTry = detour_alloc_region_from_hi(hTargetProcess, pbTarget, pbTarget + 0x40000000);
    }
#endif

    // Try anything below.
    if (pbTry == NULL) {
        pbTry = detour_alloc_region_from_hi(hTargetProcess, (PBYTE)pLo, pbTarget);
    }
    // try anything above.
    if (pbTry == NULL) {
        pbTry = detour_alloc_region_from_lo(hTargetProcess, pbTarget, (PBYTE)pHi);
    }
